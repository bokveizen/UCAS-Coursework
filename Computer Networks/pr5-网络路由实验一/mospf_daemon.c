...
void db_print()
{
	mospf_db_entry_t *db_t = NULL;
	list_for_each_entry(db_t, &mospf_db, list)
	{
		for(int i = 0; i < db_t->nadv; ++i)
		{
			fprintf(stdout, "%d.%d.%d.%d\t%d.%d.%d.%d\t%d.%d.%d.%d\t%d.%d.%d.%d\n",
			fmt(db_t->rid),
			fmt((db_t->array)[i].subnet),
			fmt((db_t->array)[i].mask),
			fmt((db_t->array)[i].rid));
		}
	}
}
