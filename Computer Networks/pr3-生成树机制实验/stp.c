...
bool cmp_with_incoming_config(stp_port_t *p, struct stp_config *config)
//True表示p的优先级高于接受到的config
{
...
bool cmp_2_ports(stp_port_t *p0, stp_port_t *p1)
//True表示p0的优先级更高
{
	...
	//如果两者到根节点的上一跳节点不同
		...
		//则上一跳节点ID小的一方优先级高
	else if (p0->designated_port != p1->designated_port)
	//如果两者到根节点的上一跳端口不同
		...
		struct stp_config *config)
{
	...
	if(!cmp_with_incoming_config(p, config))//接受到的config信息比自身优先级更高
	{
		...
		p->designated_root = ntohll(config->root_id);
	 	...
	 	p->designated_cost = ntohl(config->root_path_cost);
	  	...
	  	for (int i = 0; i < stp->nports; ++i)
	  	//遍历所有端口，找出根端口(root port)
	 	{
	 		if(stp_port_is_designated(&stp->ports[i]))
	 			continue;
	 		else if (!root_port)
	 			root_port = &stp->ports[i];
	 		else if (cmp_2_ports(&stp->ports[i], root_port))
	 			root_port = &stp->ports[i];
	 	}
	 	if (!root_port)
	 	//如果不存在根端口，则该节点为根节点
	 	...
	 	for (int i = 0; i < stp->nports; ++i)
	 	{
	 		...
	 		//如果一个端口为非指定端口，且其网段通过本节点到根节点的代价比
	 		//通过对端节点的代价小，那么该端口成为指定端口，并更新如下内容
	 		{
	 			stp->ports[i].designated_root = stp->designated_root;
				stp->ports[i].designated_cost = stp->root_path_cost;
