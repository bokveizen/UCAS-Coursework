...
void handle_packet(iface_info_t *iface, char *packet, int len)
{
	...
	iface_info_t* dst = lookup_port(eh->ether_dhost);
	printf("dst lookup OK!\n\n");
	iface_info_t* src = lookup_port(eh->ether_shost);
	printf("src lookup OK!\n\n");
	if(!dst)
	{
	...
	{
		iface_send_packet(dst, packet, len);
	}
	if(!src)
	{
