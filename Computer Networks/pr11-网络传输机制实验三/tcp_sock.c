...
struct tcp_sock *alloc_tcp_sock()
{
	...
	init_list_head(&tsk->send_list);
	init_list_head(&tsk->recv_list);
	pthread_mutex_init(&tsk->send_lock, NULL);
	pthread_mutex_init(&tsk->timer_lock, NULL);
...
struct tcp_sock *tcp_sock_lookup_established(u32 saddr, u32 daddr, u16 sport, u16 dport)
{
	...
	struct tcp_sock *tgt;
	list_for_each_entry(tgt, list, hash_list) {
		if (saddr == tgt->sk_sip &&
			daddr == tgt->sk_dip &&
			sport == tgt->sk_sport &&
			dport == tgt->sk_dport)
...
struct tcp_sock *tcp_sock_lookup_listen(u32 saddr, u16 sport)
{
	...
	struct tcp_sock *tgt;
	list_for_each_entry(tgt, list, hash_list){
		if(tgt->sk_sport == sport) return tgt;
	}
...
int tcp_sock_write(struct tcp_sock *tsk, char *buf, int len)
{
	...
	while(1){
		...
		{
			char* send_buf;
			send_buf = buf + len - len_to_go;
			int send_length, send_length_t;
			if(less_or_equal_32b(tsk->snd_wnd, (u32)len_to_go)){
				send_length_t = (int)tsk->snd_wnd;
			}
			else{
				send_length_t = len_to_go;
			}

			if(send_length_t < 1500 - ETHER_HDR_SIZE - IP_BASE_HDR_SIZE - TCP_BASE_HDR_SIZE){
				send_length = send_length_t;
			}
			else{
				send_length = 1500 - ETHER_HDR_SIZE - IP_BASE_HDR_SIZE - TCP_BASE_HDR_SIZE;
			}
			len_to_go -= send_length;
			
			char* pkt;
			pkt = (char *)malloc(ETHER_HDR_SIZE + IP_BASE_HDR_SIZE + TCP_BASE_HDR_SIZE + send_length);
						
			char* data_area;
			data_area = pkt + ETHER_HDR_SIZE + IP_BASE_HDR_SIZE + TCP_BASE_HDR_SIZE;
			memcpy(data_area, send_buf, send_length);

			tcp_send_packet(tsk, pkt, ETHER_HDR_SIZE + IP_BASE_HDR_SIZE + TCP_BASE_HDR_SIZE + send_length);
			if(len_to_go <= 0) return len;
		}
		else{
...
int tcp_sock_read(struct tcp_sock *tsk, char *buf, int len)
{
	...
	pthread_mutex_lock(&(tsk->rcv_buf->lock));
	int rcv_buf_empty = ring_buffer_empty(tsk->rcv_buf);
	pthread_mutex_unlock(&(tsk->rcv_buf->lock));
	if(rcv_buf_empty) sleep_on(tsk->wait_recv);
	pthread_mutex_lock(&(tsk->rcv_buf->lock));
	if(get_last())
	{
		int size = read_ring_buffer(tsk->rcv_buf, buf, len);
