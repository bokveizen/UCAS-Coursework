...
static void initialize_pcb(pcb_t *p, pid_t pid, struct task_info *ti)
{
     ...
     raw_priority[pid] = priotity_cal(pid);
     act_priority[pid] = priotity_cal(pid);
...
void print_status(void)
{
     ...
     printf(base - 2, 1, "Pid\tType\tStatus\tEntries\tPriority");
     for (i = 0; i < NUM_PCBS && (base + i) < 25; i++) {
          printf(base + i, 1, "%d\t%s\t%s\t%d\t%d\t%d", pcb[i].pid,
                 pcb[i].task_type == KERNEL_THREAD ? "Thread" : "Process",
                 status[pcb[i].status], pcb[i].entry_count, raw_priority[i]);
     }
}
